package com.mertpolat.detection;
...
import com.mertpolat.detection.env.ImageUtils;
import com.mertpolat.detection.env.Logger;
...
public abstract class CameraActivity extends Activity
    ...
    private void nesneTanı() {

        final double sagBaslangic = previewWidth / 2 - 0.10 * previewWidth;
        final double sagBitis = previewWidth;
        final double baslangic = 0;
        final double solBitis = previewWidth / 2 + 0.10 * previewWidth;
        final double onIzlemeAlanı = previewWidth * previewHeight;
        ...
        for (int i = 0; i < currentRecognitions.size(); i++) {
            ...
            float basla = recognition.getLocation().top;
            float son = recognition.getLocation().bottom;
            double nesneAlanı = recognition.getLocation().width() * recognition.getLocation().height();

            if (nesneAlanı > onIzlemeAlanı / 2) {
                stringBuilder.append(" senin Önünde ");
            } else {


                if (basla > baslangic && son < solBitis) {
                    stringBuilder.append(" sağında ");
                } else if (basla > sagBaslangic && son < sagBitis) {
                    stringBuilder.append(" solunda ");
                } else {
                    stringBuilder.append(" senin önünde ");
                }
            }
        ...
        stringBuilder.append(" algılandı.");
